...
lm.ineq <- lm(Index.HS ~ Income.inequality, data = Index_inequality_df)
# lm.ineq <- lm(Index_inequality_df[2:1])
...
mtext(c("좋음", "나쁨"), side = 2, at = c(-1.8, 2.8), las = 1)
text(x = 5, y = 1.5, labels = paste("r =", round(cor(Index_inequality_df[1], Index_inequality_df[2]), digits = 2)))
lsfit.ineq <- lsfit(x = Index_inequality_df[, 1], y = Index_inequality_df[, 2])
...
title(main = main_title_2.kr, xlab = x_lab_2.kr, ylab = y_lab_2.kr)
mtext(c("좋음", "나쁨"), side = 2, at = c(-1.8, 2.8), las = 1)
